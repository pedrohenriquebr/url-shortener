    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
            try
            {
                ...
                await foreach (var key in keys) keysToProcess.Add(key);

                if (!keysToProcess.Any()) continue;
                ...
                for (var i = 0; i < keysToProcess.Count; i++)
                {
                    var shortCode = keysToProcess[i].ToString().Replace(AccessCountKeyPrefix, "");
                    if (long.TryParse(values[i], out var count)) updateTasks[shortCode] = count;
                }
